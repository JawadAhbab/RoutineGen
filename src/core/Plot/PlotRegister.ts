...
export class PlotRegister {
  ...
  public createHTML() {
    ...
    timeline.forEach((task, idx) => {
      ...
      if (!lastTask) return (frags[fragidx] = [task])
      if (lastTask.ID === task.ID) return (lastTask.portion += 1)
      frags[fragidx].push(task)
    })

    return frags
      .map((frag) => `<div class="hour">\n${frag.map((i) => i.getHTML()).join('\n')}\n</div>`)
      .join('\n')
  }
}
